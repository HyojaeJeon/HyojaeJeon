 ...
 *   → g_TableMgr->InitTable() + g_TableMgr->m_TableLst duyệt
 */
...
public:
    // ──────────────────────────────────────────
    // Constructor — Chỉ cần TableMgr (read-only UseCase)
    // Constructor — TableMgr만 필요 (읽기 전용 UseCase)
    //
    ...
    // ──────────────────────────────────────────
    explicit RefreshTablesUseCase(TableMgr* table_mgr);
    ...
    // ──────────────────────────────────────────
    UseCaseResult Execute(const RequestContext& ctx, int floor_id = 0);

private:
    TableMgr* table_mgr_;  // non-owning
};
