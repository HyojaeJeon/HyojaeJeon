...
#include "../Shared/TransactionRunner.h"
#include "../Shared/IdempotencyService.h"
#include "../Shared/OperationLockService.h"
...
SelectTableUseCase::SelectTableUseCase(
    TableMgr*           table_mgr,
    TransactionRunner*  tx_runner,
    IdempotencyService* idempotency,
    PosRealTimeSender*  realtime_sender)
    : table_mgr_(table_mgr)
    , tx_runner_(tx_runner)
    , idempotency_(idempotency)
    , realtime_sender_(realtime_sender)
{
    // ──────────────────────────────────────────
...
UseCaseResult SelectTableUseCase::Execute(const RequestContext& ctx, int table_code) {
    ...
    if (!ctx.idempotency_key.empty()) {
        auto check = idempotency_->Check(ctx.idempotency_key, ctx.request_id);

        switch (check) {
            case IdempotencyService::CheckResult::DUPLICATE_SUCCESS: {
                // Đã xử lý thành công trước đó → trả lại kết quả cũ
                // 이전에 성공 처리됨 → 기존 결과 재반환
                auto prev = idempotency_->GetPreviousResult(ctx.idempotency_key);
        ...
        idempotency_->MarkProcessing(ctx.idempotency_key, ctx.request_id);
    }
    ...
    OperationLockService::Guard lock(
        ...
        "TABLE",                                    // domain
        std::to_string(table_code)                  // entity_id
    );
    ...
    if (!lock.Acquired()) {
        ...
        return UseCaseResult::Busy(
            "Table " + std::to_string(table_code) + " is being processed"
        );
    }
    ...
    // ──────────────────────────────────────────
    auto result = tx_runner_->Run([&]() -> UseCaseResult {
        ...
        std::string data_json =
            "{\"tableCode\":" + std::to_string(table_code) +
            ",\"status\":\"" + table_result.status + "\"}";

        return UseCaseResult::Success(data_json);
    });
    ...
    if (!ctx.idempotency_key.empty()) {
        if (result.ok) {
            idempotency_->MarkSucceeded(ctx.idempotency_key, result.data_json);
        } else {
            idempotency_->MarkFailed(ctx.idempotency_key, result.code);
        }
    }
    ...
    // ──────────────────────────────────────────
    if (result.ok) {
        realtime_sender_->NotifyTableChanged();
    }
