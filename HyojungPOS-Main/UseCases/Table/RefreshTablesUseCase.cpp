...
RefreshTablesUseCase::RefreshTablesUseCase(TableMgr* table_mgr)
...
UseCaseResult RefreshTablesUseCase::Execute(const RequestContext& ctx, int floor_id) {
    ...
    auto tables = table_mgr_->GetAllTables(floor_id);
    ...
    json << "],\"maxFloor\":" << table_mgr_->GetMaxFloor() << "}";
