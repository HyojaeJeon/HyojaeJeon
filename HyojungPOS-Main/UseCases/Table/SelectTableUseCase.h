 ...
 *     3. Gọi TableMgr để chiếm bàn (quy tắc nghiệp vụ)
 *        TableMgr를 호출하여 테이블 점유 (업무 규칙)
 ...
 *   → g_TableMgr->SetUseSysn(...)
 *   → g_TableMgr->GetTableFromTableCode(...)
 *   → UpdateUI trực tiếp
 *
...
#include "../Shared/UseCaseResult.h"
#include "../Shared/RequestContext.h"
...
public:
    ...
    // @param table_code  Mã bàn cần chọn / 선택할 테이블 코드
    // @return UseCaseResult — Kết quả chuẩn / 표준 결과
    // ──────────────────────────────────────────
    UseCaseResult Execute(const RequestContext& ctx, int table_code);
...
private:
    ...
    // ──────────────────────────────────────────
    TableMgr*           table_mgr_;        // Domain: Quy tắc nghiệp vụ bàn / 테이블 업무 규칙
    TransactionRunner*  tx_runner_;         // Shared: Transaction wrapper / 트랜잭션 래퍼
    IdempotencyService* idempotency_;       // Shared: Kiểm tra trùng lặp / 중복 검사
    PosRealTimeSender*  realtime_sender_;   // Presentation: UI event / UI 이벤트 전송
};
