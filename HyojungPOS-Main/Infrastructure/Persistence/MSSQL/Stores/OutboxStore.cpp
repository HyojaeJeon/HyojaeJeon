...
std::vector<OutboxEntry> OutboxStore::FindPending(int limit)
{
    ...
    const std::string& error)
{
    ...
    for (auto& entry : s_outbox) {
        if (entry.id == id) {
            ...
            if (new_status == "FAILED_RETRYABLE" || new_status == "FAILED_TERMINAL") {
                entry.retry_count++;
            }
...
int OutboxStore::GetBacklogCount()
{
