...
public:
    explicit PosRealTimeSender(CefBrowserDlg* browser_dlg);
    ...
    /** 테이블 상태 변경 알림 */
    void NotifyTableChanged(int floor_id = 0);

    /** 새 주문 알림 */
    void NotifyNewOrder(const std::string& order_id, const std::string& table_id);

    /** 결제 완료 알림 */
    void NotifyPaymentComplete(const std::string& payment_id,
                               const std::string& order_id);

    /** 동기화 상태 변경 알림 */
    void NotifySyncStatusChanged(int backlog_count,
...
private:
    CefBrowserDlg* browser_dlg_;  // non-owning
