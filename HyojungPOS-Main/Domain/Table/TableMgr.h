#pragma once
/**
 * TableMgr — Quy tắc nghiệp vụ bàn / Quản lý trạng thái bàn
 * TableMgr — 테이블 업무 규칙 / 상태 변경 관리
 *
 ...
 *   HJ-POS-TEST/DB/TableMgr.h — CTableMgr : public CMgr
 *   Chức năng từ CTableMgr được chuyển về đây,
 *   nhưng loại bỏ phần UI (pBtn, pImgBtn, pTableDlg) và
 *   phần trách nhiệm của UseCase (transaction, UI notification).
 *   기존 CTableMgr의 기능을 가져오되,
...
#include "../../Infrastructure/Persistence/MSSQL/Rows/Table/TableRow.h"
...
public:
    ...
    //
    // Tương đương: CTableMgr::InitTable() phần SELECT
    // 대응: CTableMgr::InitTable()의 SELECT 부분
    //
    // @param floor_id  Lọc theo tầng (0 = tất cả) / 층별 필터 (0 = 전체)
    // @return Danh sách TableRow / TableRow 목록
    // ──────────────────────────────────────────
    std::vector<TableRow> GetAllTables(int floor_id = 0);

    // ──────────────────────────────────────────
    // GetTableByCode — Lấy thông tin 1 bàn theo mã
    // GetTableByCode — 테이블 코드로 1개 테이블 조회
    //
    // Tương đương: CTableMgr::GetTableFromTableCode(int)
    // 대응: CTableMgr::GetTableFromTableCode(int)
    //
    // @param table_code  Mã bàn / 테이블 코드
    // @return TableRow hoặc nullopt nếu không tìm thấy
    //         TableRow 또는 찾지 못하면 nullopt
    // ──────────────────────────────────────────
    std::optional<TableRow> GetTableByCode(int table_code);

    // ──────────────────────────────────────────
    // GetMaxFloor — Lấy số tầng lớn nhất
    // GetMaxFloor — 최대 층 번호 조회
    //
    // Tương đương: CTableMgr::m_nMaxFloor
    // 대응: CTableMgr::m_nMaxFloor
    // ──────────────────────────────────────────
    int GetMaxFloor();
    ...
    //
    // Tương đương: CTableMgr::SetUseSysn(UseSysn, TRUE, TableCode) phần logic
    // 대응: CTableMgr::SetUseSysn(UseSysn, TRUE, TableCode) 중 로직 부분
    //
    // @param table_code  Mã bàn / 테이블 코드
    // @return TableResult
    // ──────────────────────────────────────────
    ...
    //
    // Tương đương: CTableMgr::ChangeTable(tTable*, tTable*)
    // 대응: CTableMgr::ChangeTable(tTable*, tTable*)
    //
    ...
    //
    // Tương đương: CTableMgr::UseSysnCheck()
    // 대응: CTableMgr::UseSysnCheck()
    // ──────────────────────────────────────────
