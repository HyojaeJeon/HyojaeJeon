...
#include "../../Infrastructure/Persistence/MSSQL/Sql/Table/TableSql.h"
#include "../../Infrastructure/Persistence/MSSQL/Mappers/Table/TableRowMapper.h"
#include "../../Infrastructure/Persistence/MSSQL/Schema/Table/TableSchema.h"
...
TableMgr::~TableMgr() {
    ...
    //
    // Sau refactoring: Dùng std::vector<TableRow> nên không cần xóa thủ công
    // 리팩토링 후: std::vector<TableRow>을 사용하므로 수동 해제 불필요
}
...
std::vector<TableRow> TableMgr::GetAllTables(int floor_id) {
    // ──────────────────────────────────────────
    // Tương đương code cũ: CTableMgr::InitTable()
    // 대응하는 기존 코드: CTableMgr::InitTable()
    //
    // Code cũ:
    //   sql.Format(_T("Select FloorNum,LineNum,ColNum,TableType,...
    //     From TableMgr WHERE DelFlag is NULL Order by FloorNum,PageNum"));
    ...
    //
    // Sau refactoring:
    //   - SQL được tạo bởi TableSql (Infrastructure tầng)
    //     SQL은 TableSql(Infrastructure 계층)이 생성
    //   - Mapping được xử lý bởi TableRowMapper
    //     매핑은 TableRowMapper가 처리
    ...
    // 실제 통합 시 코드는 아래와 같다:
    //
    // std::string sql = TableSql::SelectAll(floor_id);
    // CADORecordset rs(g_DbAccess->GetDatabase());
    // if (!rs.OpenQuery(sql.c_str())) return result;
    // while (!rs.IsEOF()) {
    //     result.push_back(TableRowMapper::FromRecordset(rs));
    //     rs.MoveNext();
    // }
...
std::optional<TableRow> TableMgr::GetTableByCode(int table_code) {
    // ──────────────────────────────────────────
    // Tương đương code cũ: CTableMgr::GetTableFromTableCode(int)
    // 대응하는 기존 코드: CTableMgr::GetTableFromTableCode(int)
    //
    ...
    // TODO: 실제 ADO 통합
    //
    // std::string sql = TableSql::SelectByTableCode(table_code);
    // CADORecordset rs(g_DbAccess->GetDatabase());
    // if (!rs.OpenQuery(sql.c_str()) || rs.IsEOF()) return std::nullopt;
    // return TableRowMapper::FromRecordset(rs);

    return std::nullopt;
}

int TableMgr::GetMaxFloor() {
    // ──────────────────────────────────────────
    // Tương đương code cũ: CTableMgr::m_nMaxFloor
    // 대응하는 기존 코드: CTableMgr::m_nMaxFloor
    //
    ...
    // TODO: 실제 ADO 통합
    //
    // std::string sql = "SELECT ISNULL(MAX(FloorNum), 1) FROM TableMgr WHERE DelFlag IS NULL";
    // CADORecordset rs(g_DbAccess->GetDatabase());
...
TableResult TableMgr::OccupyTable(int table_code) {
    // ──────────────────────────────────────────
    // Chiếm bàn (chuyển sang "đang dùng")
    // 테이블 점유 ("사용중"으로 변경)
    //
    // Tương đương code cũ:
    //   CTableMgr::SetUseSysn(UseSysn, TRUE, TableCode)
    ...
    if (!table.has_value()) {
        ...
        result.error_detail = "Table code " + std::to_string(table_code) + " not found";
        return result;
    }
    ...
    if (table->status == TableStatus::OCCUPIED) {
        ...
        result.error_detail = "Table " + std::to_string(table_code) + " is already occupied";
        return result;
    }
    ...
    if (table->status == TableStatus::LOCKED) {
        ...
        result.error_detail = "Table " + std::to_string(table_code) + " is locked";
        return result;
    }
    ...
    // TODO: 실제 ADO 통합
    //
    // std::string sql = TableSql::UpdateStatus(table_code, TableStatus::OCCUPIED);
...
TableResult TableMgr::ReleaseTable(int table_code) {
    // ──────────────────────────────────────────
    ...
    //   Dlg tự gọi g_TableMgr để đổi trạng thái bàn.
    //
    // 대응하는 기존 코드:
    //   AccountDlg::OnBtnPayment() 끝 부분 — 결제 완료 후
    //   Dlg가 직접 g_TableMgr를 호출하여 테이블 상태를 변경했었음.
    //
    ...
    if (table->status == TableStatus::EMPTY) {
        ...
        result.error_detail = "Table " + std::to_string(table_code) + " is already empty";
        return result;
    }
    ...
    // TODO: 실제 ADO 통합
    //
    // std::string sql = TableSql::UpdateStatus(table_code, TableStatus::EMPTY);
...
TableResult TableMgr::ChangeTable(int from_table_code, int to_table_code) {
    // ──────────────────────────────────────────
    // Di chuyển đơn hàng từ bàn cũ sang bàn mới
    // 기존 테이블에서 새 테이블로 주문 이동
    //
    // Tương đương code cũ: CTableMgr::ChangeTable(tTable*, tTable*)
    // 대응하는 기존 코드: CTableMgr::ChangeTable(tTable*, tTable*)
    //
    ...
    // TODO: 실제 ADO 통합
    //
    // std::string sql1 = TableSql::UpdateStatus(from_table_code, TableStatus::EMPTY);
    // std::string sql2 = TableSql::UpdateStatus(to_table_code, TableStatus::OCCUPIED);
...
bool TableMgr::IsOccupied(int table_code) {
    // Tương đương code cũ: CTableMgr::UseSysnCheck()
    // 대응하는 기존 코드: CTableMgr::UseSysnCheck()
    auto table = GetTableByCode(table_code);
    return table.has_value() && table->status == TableStatus::OCCUPIED;
}

bool TableMgr::IsLocked(int table_code) {
    auto table = GetTableByCode(table_code);
