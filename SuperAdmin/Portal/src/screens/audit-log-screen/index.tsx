...
export function AuditLogScreen() {
  ...
  const columns: DataTableColumn<AuditLogRow>[] = [
    ...
    { key: 'actorId', header: 'Actor ID', width: '120px', render: (r) => <span className="font-mono text-[11px] text-fg-subtle">{r.actorId?.slice(0, 8) ?? '—'}</span> },
    { key: 'action', header: 'Action', render: (r) => <span className="font-mono text-[12px]">{r.actionType}</span> },
    { key: 'target', header: 'Target', render: (r) => <span className="text-[12px]"><span className="text-fg">{r.targetType}</span>{r.targetId && <span className="text-fg-subtle"> · {r.targetId.slice(0, 8)}</span>}</span> },
  ];
  ...
  return (
    ...
    >
      <SectionCard
        title={t('audit.list.title')}
        description={`${rows.length} events · ${pageInfo?.hasNextPage ? 'more available' : 'end'}`}
        padding="none"
        footer={
