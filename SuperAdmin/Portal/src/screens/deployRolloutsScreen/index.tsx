...
export function DeployRolloutsScreen() {
  ...
  const columns: DataTableColumn<DeployPackageRow>[] = [
    { key: 'code', header: t('deploy.package'), render: (r) => <span className="font-mono text-[12px]">{r.packageCode}</span> },
    { key: 'version', header: t('deploy.version'), width: '120px', render: (r) => <span className="font-mono text-[12px]">{r.version}</span> },
    { key: 'target', header: t('deploy.target'), width: '140px', render: (r) => <Badge tone="info" variant="soft">{r.platformTarget}</Badge> },
    {
      key: 'state',
      header: t('deploy.rolloutState'),
      width: '160px',
      render: (r) => r.releasedAt ? <Badge tone="success" startDot>{t('deploy.released')}</Badge> : <Badge tone="warning" startDot>{t('deploy.draft')}</Badge>,
    },
    { key: 'released', header: t('deploy.releasedAt'), render: (r) => r.releasedAt ? <span className="num text-[12px] text-fg-muted">{formatDateTime(r.releasedAt)}</span> : <span className="text-fg-subtle">—</span> },
  ];
  ...
  return (
    <ListPageTemplate
      header={{
        ...
        description: t('deploy.rollouts.description'),
    ...
    >
      <SectionCard
        title={<span className="flex items-center gap-2"><Activity size={14} className="text-primary" />{t('deploy.rollouts.sectionTitle')}</span>}
        description={`${packages.length} ${t('deploy.rollouts.packageCount')}`}
        padding="none"
      >
