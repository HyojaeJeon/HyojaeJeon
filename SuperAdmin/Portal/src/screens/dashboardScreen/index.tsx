...
export function DashboardScreen() {
  ...
  // envelope unwrap — totalCount 사용 (서버에서 정확한 전체 카운트 반환)
  const distributorCount = (data?.distributors.success as { totalCount?: number })?.totalCount ?? null;
  const brandCount = (data?.brands.success as { totalCount?: number })?.totalCount ?? null;
  const branchCount = (data as { branches?: { success?: { totalCount?: number } } })?.branches?.success?.totalCount ?? null;
  const edgePosCount: number | null = null; // TODO: edgePosTerminals count resolver
  const corporateCount = (data as { mealCorporates?: { success?: { totalCount?: number } } })?.mealCorporates?.success?.totalCount ?? null;
  const licenseCount = (data?.licenses.success as { totalCount?: number })?.totalCount ?? null;
  ...
  const auditColumns: DataTableColumn<AuditRow>[] = [
    ...
    { key: 'actor', header: t('table.header.actor'), render: (row) => <Badge tone="neutral" variant="outline">{row.actorType}</Badge> },
    { key: 'action', header: t('table.header.action'), render: (row) => <span className="font-mono text-[12px]">{row.actionType}</span> },
    {
      key: 'target',
      header: t('table.header.target'),
      render: (row) => (
  ...
  const licenseColumns: DataTableColumn<LicenseRow>[] = [
    { key: 'code', header: t('table.header.code'), render: (row) => <span className="font-mono text-[12px]">{row.licenseCode}</span> },
    { key: 'type', header: t('table.header.type'), render: (row) => <span className="text-[12px]">{t(`enum.licenseType.${row.licenseType}`)}</span> },
    { key: 'scope', header: t('table.header.scope'), render: (row) => <Badge tone="neutral" variant="soft">{t(`enum.scopeType.${row.scopeType}`)}</Badge> },
    { key: 'status', header: t('table.header.status'), render: (row) => <Badge tone={statusTone(row.status)} startDot>{t(`enum.status.${row.status}`)}</Badge> },
    {
      key: 'expires',
