...
export function OperationsOverviewScreen() {
  ...
  const syncCols: DataTableColumn<{ id: string; eventType: string; edgePosId: string | null; createdAt: string }>[] = [
    { key: 'when', header: t('table.header.when'), width: '130px', render: (r) => <span className="num text-[12px] text-fg-muted" title={formatDateTime(r.createdAt, tag)}>{formatRelativeTime(r.createdAt, tag)}</span> },
    { key: 'event', header: t('table.header.event'), render: (r) => <span className="font-mono text-[12px]">{r.eventType}</span> },
    { key: 'edge', header: t('table.header.edgePos'), width: '200px', render: (r) => <span className="font-mono text-[11px] text-fg-muted">{r.edgePosId ?? '—'}</span> },
  ];

  const auditCols: DataTableColumn<{ id: string; createdAt: string; actorType: string; actionType: string; targetType: string }>[] = [
    { key: 'when', header: t('table.header.when'), width: '130px', render: (r) => <span className="num text-[12px] text-fg-muted">{formatRelativeTime(r.createdAt, tag)}</span> },
    { key: 'actor', header: t('table.header.actor'), width: '160px', render: (r) => <Badge tone="neutral" variant="outline">{r.actorType}</Badge> },
    { key: 'action', header: t('table.header.action'), render: (r) => <span className="font-mono text-[12px]">{r.actionType}</span> },
    { key: 'target', header: t('table.header.target'), width: '160px', render: (r) => <span className="text-[12px]">{r.targetType}</span> },
  ];
