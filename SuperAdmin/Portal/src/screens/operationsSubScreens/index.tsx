  ...
  return (
    <DashboardPageTemplate
      header={{
        ...
        description: t('ops.realtime.description'),
      ...
      primary={
        <SectionCard title={<span className="flex items-center gap-2"><Radar size={14} className="text-primary" />{t('ops.topicActivity')}</span>} description={t('ops.topicActivityDesc')}>
              ...
              {TOPICS.map((topic) => {
                ...
                return (
                  <div key={topic} className="flex items-center justify-between rounded-md border bg-surface-1 p-2.5" style={{ borderColor: 'var(--border)' }}>
                    <span className="text-[11px] text-fg-muted">{t(`ops.topic.${topic}`, topic)}</span>
...
export function OperationsTelemetryScreen() {
  ...
  const columns: DataTableColumn<(typeof rows)[number]>[] = [
    { key: 'edge', header: t('ops.edgePosId'), render: (r) => <span className="font-mono text-[11px]">{r.id}</span> },
    { key: 'count', header: t('ops.events'), width: '100px', align: 'right', render: (r) => <span className="num font-mono text-[12px]">{r.count}</span> },
    { key: 'last', header: t('ops.lastEvent'), render: (r) => <Badge tone="info" variant="soft">{r.lastEvent}</Badge> },
    { key: 'at', header: t('ops.at'), width: '160px', render: (r) => <span className="num text-[12px] text-fg-muted" title={formatDateTime(r.lastAt, tag)}>{formatRelativeTime(r.lastAt, tag)}</span> },
  ];

  const metrics: SharedUiStat[] = [
    { label: t('ops.uniqueEdgePos'), value: loading ? <Skeleton width={40} height={22} /> : rows.length, tone: 'brand' },
    { label: t('ops.recentEvents'), value: loading ? <Skeleton width={40} height={22} /> : events.length, tone: 'info' },
  ];
  ...
  return (
    <DashboardPageTemplate
      header={{
        ...
        description: t('ops.telemetry.description'),
      ...
      metrics={metrics}
      primary={
        <SectionCard title={<span className="flex items-center gap-2"><MonitorDot size={14} className="text-primary" />{t('ops.terminalsByActivity')}</span>} padding="none">
          {loading && rows.length === 0 ? (
            <div className="space-y-2 p-4">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} height={28} />)}</div>
          ) : (
            <DataTable columns={columns} rows={rows} rowKey={(r) => r.id} compact emptyState={t('ops.emptyTerminalActivity')} />
          )}
        </SectionCard>
      }
    />
...
export function OperationsIncidentsScreen() {
  ...
  const columns: DataTableColumn<AuditLogRow>[] = [
    { key: 'when', header: t('ops.when'), width: '140px', render: (r) => <span className="num text-[12px] text-fg-muted" title={formatDateTime(r.createdAt, tag)}>{formatRelativeTime(r.createdAt, tag)}</span> },
    { key: 'actor', header: t('ops.actor'), width: '160px', render: (r) => <Badge tone="neutral" variant="outline">{r.actorType}</Badge> },
    { key: 'action', header: t('ops.action'), render: (r) => <span className="font-mono text-[12px] text-danger">{r.actionType}</span> },
    { key: 'target', header: t('ops.target'), width: '200px', render: (r) => <span className="text-[12px]"><span className="text-fg">{r.targetType}</span>{r.targetId && <span className="text-fg-subtle"> · {r.targetId.slice(0, 8)}</span>}</span> },
  ];
  ...
  return (
    <DashboardPageTemplate
      header={{
        ...
        description: t('ops.incidents.description'),
      ...
      primary={
        <SectionCard title={<span className="flex items-center gap-2"><AlertTriangle size={14} className="text-warn" />{t('ops.recentIncidents')}</span>} description={`${incidents.length} / ${edges.length} ${t('ops.auditEvents')}`} padding="none">
          {loading && incidents.length === 0 ? (
            <div className="space-y-2 p-4">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} height={28} />)}</div>
          ) : (
            <DataTable columns={columns} rows={incidents} rowKey={(r) => r.id} compact emptyState={t('ops.emptyIncidents')} />
          )}
        </SectionCard>
      }
    />
