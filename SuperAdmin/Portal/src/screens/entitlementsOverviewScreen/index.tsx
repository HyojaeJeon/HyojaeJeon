...
type ModalType = 'grant' | 'suspend' | 'resume' | 'revoke' | null;
...
export function EntitlementsOverviewScreen() {
  ...
  const [selectedEntitlement, setSelectedEntitlement] = useState<BrandEntitlementRow | null>(null);
  ...
  const openModal = (type: ModalType, ent?: BrandEntitlementRow) => {
  ...
  const renderActions = (r: BrandEntitlementRow) => {
    const status = r.status;
    if (status === 'ACTIVE' || status === 'TRIAL') {
      return (
        <div style={{ display: 'flex', gap: 4 }}>
          <Button variant="ghost" size="sm" onClick={() => openModal('suspend', r)}>
            {t('entitlement.action.suspend')}
          </Button>
          <Button variant="ghost" size="sm" onClick={() => openModal('revoke', r)}>
            {t('entitlement.action.revoke')}
          </Button>
        </div>
      );
    }
    if (status === 'SUSPENDED') {
      return (
        <div style={{ display: 'flex', gap: 4 }}>
          <Button variant="ghost" size="sm" onClick={() => openModal('resume', r)}>
            {t('entitlement.action.resume')}
          </Button>
          <Button variant="ghost" size="sm" onClick={() => openModal('revoke', r)}>
            {t('entitlement.action.revoke')}
          </Button>
  ...
  const entCols: DataTableColumn<BrandEntitlementRow>[] = [
    ...
    { key: 'actions', header: t('entitlement.columns.actions'), width: '200px', render: renderActions },
  ];
    ...
    <ListPageTemplate
      ...
      filters={
        <FilterBar>
          ...
          {selectedId && (
            <Button variant="primary" size="sm" startIcon={<Plus size={14} />} onClick={() => openModal('grant')}>
    ...
    >
      ...
      {selectedId && (
        <GrantCapabilityModal
          ...
          brandHqId={selectedId}
          onClose={closeModal}
          onGranted={handleDone}
        />
      )}
      {selectedEntitlement && (
        <>
          <SuspendModal
            ...
            capability={selectedEntitlement.capability}
            onClose={closeModal}
            onDone={handleDone}
          />
          <ResumeModal
