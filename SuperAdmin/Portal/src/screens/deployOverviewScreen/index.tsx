...
export function DeployOverviewScreen() {
  ...
  const cols: DataTableColumn<{ id: string; packageCode: string; version: string; platformTarget: string; createdAt: string; releasedAt: string | null }>[] = [
    { key: 'code', header: t('table.header.code'), width: '200px', render: (r) => <span className="font-mono text-[12px]">{r.packageCode}</span> },
    { key: 'version', header: t('table.header.version'), width: '120px', render: (r) => <span className="font-mono text-[12px]">{r.version}</span> },
    { key: 'target', header: t('table.header.target'), width: '140px', render: (r) => <Badge tone="info" variant="soft">{r.platformTarget}</Badge> },
    { key: 'released', header: t('table.header.released'), render: (r) => r.releasedAt ? <span className="num text-[12px] text-fg-muted">{formatDateTime(r.releasedAt)}</span> : <Badge tone="warning" variant="soft">DRAFT</Badge> },
  ];
