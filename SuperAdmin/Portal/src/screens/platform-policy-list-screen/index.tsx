...
export function PlatformPolicyListScreen() {
  ...
  const columns: DataTableColumn<PlatformPolicyRow>[] = [
    { key: 'key', header: 'Policy key', render: (r) => <span className="font-mono text-[12px]">{r.policyKey}</span> },
    { key: 'scope', header: 'Scope', width: '200px', render: (r) => <><Badge tone="info" variant="soft">{r.scopeType}</Badge>{r.scopeId && <span className="ml-2 font-mono text-[11px] text-fg-subtle">{r.scopeId.slice(0, 8)}</span>}</> },
    { key: 'version', header: 'Version', width: '100px', align: 'right', render: (r) => <span className="num font-mono text-[12px]">v{r.version}</span> },
    ...
    { key: 'updated', header: 'Updated', width: '180px', align: 'right', render: (r) => <span className="num text-[12px] text-fg-muted">{formatDateTime(r.updatedAt)}</span> },
  ];
