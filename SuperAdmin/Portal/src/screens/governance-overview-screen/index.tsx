...
export function GovernanceOverviewScreen() {
  ...
  const metrics: SharedUiStat[] = [
    { label: t('nav.governance.licenses'), value: loading ? <Skeleton width={40} height={22} /> : licenses.length, icon: <BadgeCheck size={14} />, tone: 'brand' },
    ...
    { label: t('user.list.title'), value: loading ? <Skeleton width={40} height={22} /> : users.length, icon: <UserCog size={14} />, tone: 'success' },
  ];

  const userCols: DataTableColumn<{ id: string; loginId: string; userType: string; status: string }>[] = [
    { key: 'loginId', header: 'Login ID', render: (r) => <span className="font-mono text-[12px]">{r.loginId}</span> },
    { key: 'type', header: 'Type', width: '180px', render: (r) => <Badge tone="info" variant="soft">{r.userType}</Badge> },
  ...
  return (
    <DashboardPageTemplate
      header={{
        ...
        description: 'Licenses · Entitlements · Policies · RBAC · Users',
      ...
      primary={
        <>
          <SectionCard
            title={
              <Link href="/governance/tenant-users" className="flex items-center gap-2 text-fg hover:text-primary">
              ...
              <ul className="space-y-1 text-[12.5px]">
                {roles.slice(0, 6).map((r) => (
                  <li key={r.id} className="flex items-center justify-between">
                    <Link href={`/governance/tenant-users?tab=roles`} className="font-mono text-fg hover:text-primary">
                      {r.roleCode}
                    </Link>
