...
export function TenantsOverviewScreen() {
  ...
  const distCols: DataTableColumn<{ id: string; distributorCode: string; companyName: string; countryCode: string; status: string }>[] = [
    { key: 'code', header: t('field.code'), width: '140px', render: (r) => <span className="font-mono text-[12px]">{r.distributorCode}</span> },
    { key: 'name', header: t('field.company'), render: (r) => <span className="font-medium text-fg">{r.companyName}</span> },
      ...
      primary={
        <>
          <SectionCard
            ...
            description={`${dists.length} ${t('tenant.distributorCount')}`}
            padding="none"
          >
            {loading ? (
          ...
          <SectionCard
            ...
            description={`${brands.length} ${t('tenant.brandCount')}`}
          >
            {loading ? (
              <Skeleton height={32} />
            ) : brands.length === 0 ? (
          ...
          <SectionCard
            ...
            description={`${corps.length} ${t('tenant.corporateCount')}`}
          >
            {loading ? (
              <Skeleton height={32} />
            ) : corps.length === 0 ? (
