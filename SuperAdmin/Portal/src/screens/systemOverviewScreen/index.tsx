...
export function SystemOverviewScreen() {
  ...
  const auditCols: DataTableColumn<{ id: string; createdAt: string; actionType: string; targetType: string }>[] = [
    { key: 'when', header: t('field.when'), width: '130px', render: (r) => <span className="num text-[12px] text-fg-muted">{formatRelativeTime(r.createdAt, tag)}</span> },
    { key: 'action', header: t('field.actionLabel'), render: (r) => <span className="font-mono text-[12px]">{r.actionType}</span> },
    { key: 'target', header: t('field.target'), width: '160px', render: (r) => <span className="text-[12px]">{r.targetType}</span> },
  ];
      ...
      primary={
        <>
          <SectionCard
            ...
            description={`${audits.length} ${t('system.recent')}`}
            padding="none"
          >
            {loading ? (
