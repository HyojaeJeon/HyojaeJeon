...
export function BranchDetailScreen() {
  ...
  const canRead = useHasPermission(PERMISSIONS.BRAND_BRANCH_READ);
  ...
  const termCols: DataTableColumn<EdgePosTerminalRow>[] = [
    { key: 'code', header: 'Code', width: '180px', render: (r) => <span className="font-mono text-[12px]">{r.terminalCode}</span> },
    { key: 'name', header: 'Name', render: (r) => <span className="font-medium text-fg">{r.terminalName}</span> },
