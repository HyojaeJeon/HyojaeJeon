...
export function SyncMonitorScreen() {
  ...
  const columns: DataTableColumn<SyncEventRow>[] = [
    ...
    { key: 'event', header: 'Event', render: (r) => <Badge tone="info" variant="soft"><span className="font-mono">{r.eventType}</span></Badge> },
    { key: 'edge', header: 'EdgePos', width: '200px', render: (r) => <span className="font-mono text-[11px] text-fg-muted">{r.edgePosId ?? '—'}</span> },
    { key: 'req', header: 'Request ID', width: '200px', render: (r) => <span className="font-mono text-[11px] text-fg-subtle">{r.requestId ?? '—'}</span> },
  ];
  ...
  return (
    <ListPageTemplate
      header={{
        ...
        description: 'syncEventConnection · 10s polling',
    ...
    >
      <SectionCard
        title="Sync events"
        description={`${rows.length} event(s) · ${pageInfo?.hasNextPage ? 'more' : 'end'}`}
        padding="none"
        footer={
