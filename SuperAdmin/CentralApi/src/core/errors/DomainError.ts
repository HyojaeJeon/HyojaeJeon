...
export function defaultStatusForCode(code: string): number {
  switch (code) {
    ...
    case 'INVALID_REFRESH_SESSION':
    case 'REFRESH_SESSION_EXPIRED':
    case 'REFRESH_SESSION_REUSED':
    ...
    case 'LICENSE_BRANCH_LIMIT_EXCEEDED':
    case 'LICENSE_TERMINAL_LIMIT_EXCEEDED':
    ...
    case 'SETTLEMENT_INVALID_STATUS_TRANSITION':
      return 409;
