/**
 * 한국어: EInvoiceConsolidator — 통합 모드 빌더 (식권 플랫폼 핵심, generic).
 *
 *   책임: 한 subject (현 1차 구현은 'CORPORATE') 의 한달치 source 거래들을
 *   합산하여 단일 `EInvoice` + `EInvoiceLine[]` 로 변환한다. polymorphic
 *   `subjectType / subjectId` 를 사용하므로 향후 BRAND_HQ / BRANCH 시나리오도
 *   같은 builder 를 재사용한다.
 *
 *   상세 사양: `1.Docs/식권관리플랫폼/EInvoice-WeTax-사양.md` §4.3 통합 모드
 *
 *   입력:
 *     - subjectType / subjectId (1차 구현은 'CORPORATE')
 ...
 *
 *   출력:
 *     - status='DRAFT' 인 EInvoice 1건 (issuanceMode='CONSOLIDATED')
 *     - 합산된 EInvoiceLine N건 (그룹화 전략에 따라 1~수십건)
 *     - sellerSnapshot / buyerSnapshot / refId / 4분할 금액
 *
 *   본 모듈은 leaf 외부에서 import 하지 않는다 (`_internal/`).
 *   외부는 `EInvoiceService.generate(...)` 를 사용 (현재 service 가 동일 로직을
 *   인라인 수행, 본 consolidator 는 향후 그룹화 전략 5종 분리 시 활성화 예정).
 *
 *   TODO Phase 1 후속:
 *     - PrismaService 주입 + 트랜잭션 (모든 line 생성을 1 트랜잭션)
 *     - 그룹화 함수 5개 구현
 *     - 멱등성: 같은 (subjectType, subjectId, periodStart) 의 ACCEPTED 존재 시 throw
 *     - sellerSnapshot 은 PlatformLegalEntity.findActive(countryCode='VN') 에서 로드
 *     - buyerSnapshot 은 subject 도메인 (MealCorporate / BrandHq / ...) 에서 PII 마스킹 후 박제
 *     - SyncWorkers 의 cron 이 본 service 의 GraphQL mutation 을 호출
 *
 * Tiếng Việt: EInvoiceConsolidator — bộ tổng hợp tháng cho hoá đơn điện tử (generic).
 */
...
export interface ConsolidationInput {
  /** 'CORPORATE' | 'BRAND_HQ' | 'BRANCH' | 'EDGE_POS' (1차 구현은 CORPORATE 단독). */
  subjectType: string;
  subjectId: string;
  ...
  periodEnd: Date;
  /** undefined 면 subject 도메인의 default consolidationStrategy 사용. */
...
export class EInvoiceConsolidator {
  ...
  /**
   * 한국어: refId 생성 — `MC` + `yyyyMM` + `subjectId-12` (CONSOLIDATED 모드).
   *
   *   사양서 §5.2 통합 모드 RefId.
   *   같은 subject 의 같은 달 → 같은 refId → @@unique 로 dedupe.
   */
  static generateRefId(subjectId: string, periodStart: Date): string {
    const yyyy = periodStart.getUTCFullYear();
    const mm = String(periodStart.getUTCMonth() + 1).padStart(2, '0');
    const sub12 = subjectId.replace(/-/g, '').slice(0, 12).toUpperCase();
    return `MC${yyyy}${mm}${sub12}`;
  }
