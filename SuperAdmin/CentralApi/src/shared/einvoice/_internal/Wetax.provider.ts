...
import { KycLookupService } from './KycLookup';
...
export class WeTaxProvider implements EInvoiceProvider {
  ...
  isOperational(config: EInvoiceProviderConfig): boolean {
    return Boolean(
      config.baseUrl &&
      config.credentialsVaultRef &&
      process.env.WETAX_PROVIDER_ENABLED === 'true' &&
      process.env.WETAX_IMPLEMENTATION_READY === 'true',
    );
  }
  ...
  async publish(ctx: EInvoicePublishContext): Promise<EInvoicePublishResult> {
    if (!this.isOperational(ctx.providerConfig)) {
      throw new DomainError({
        code: 'FEATURE_NOT_AVAILABLE',
        details: {
          providerType: this.type,
          reason: 'WeTax provider is not enabled or implementation is not marked ready',
        },
      });
    }
