...
import { CallerCtx, withTargetCorporate, withTargetBrand } from '@core/tenancy/caller-ctx';
...
} from './_internal/einvoice-provider.interface';
...
export class EInvoiceService {
  ...
  constructor(
    ...
    private readonly wetax: WeTaxProvider,
  ) {}

  private buildProviderConfig(): EInvoiceProviderConfig {
    return {
      providerType: 'WETAX',
      environment: 'SANDBOX',
      baseUrl: process.env.WETAX_BASE_URL ?? 'https://apitest.wetax.com.vn',
      credentialsVaultRef: process.env.WETAX_VAULT_REF ?? 'vault://wetax/sandbox/credentials',
      defaultSerialPrefix: 'C',
      defaultFormNo: '1',
      defaultSerialType: 'TKT',
  ...
  async listBySubject(ctx: CallerCtx, subjectType: string, subjectId: string) {
  ...
  listByCorporate(ctx: CallerCtx, corporateId: string) {
    return this.listBySubject(ctx, 'CORPORATE', corporateId);
  }
   ...
   */
  async generate(ctx: CallerCtx, input: GenerateMealConsolidatedInvoiceInput) {
  ...
  async submitForIssuance(ctx: CallerCtx, invoiceId: string) {
    ...
    if (!this.wetax.isOperational(providerConfig)) {
      throw new DomainError({
        code: 'FEATURE_NOT_AVAILABLE',
        details: {
          providerType: providerConfig.providerType,
          reason: 'e-invoice provider is not operational in this environment',
        },
      });
    }
  ...
  async runPublishAttempt(invoiceId: string) {
    ...
    // 최소 ctx 조립 — provider config 는 향후 EInvoiceProviderConfig 테이블에서 로드.
    // 1차 구현에서는 SANDBOX 기본값을 사용 (실제 발급은 production credential 주입 시점에 활성).
    const providerConfig = this.buildProviderConfig();
    ...
    try {
      const result = await this.wetax.publish(ctx);
      logRequest = result.rawRequestJson;
      logResponse = result.rawResponseJson;
      providerResponseForRow = result.rawResponseJson;
      if (result.providerStatus === 'ACCEPTED') {
        logStatus = 'ACCEPTED';
        outcome = 'ACCEPTED';
        acceptedReceiptNo = result.lookupCode ?? result.refId;
        // serial / invoice no 갱신
        await this.prisma.eInvoice.update({
          where: { id: invoiceId },
          data: {
        ...
        logStatus = 'REJECTED';
        outcome = 'REJECTED';
        logErrorMessage = result.providerMessage ?? 'Provider returned REJECTED';
      }
    } catch (e) {
