...
import {
  queueClearRefreshTokenCookie,
  queueRefreshTokenCookie,
  readRefreshTokenCookie,
  queueAccessTokenCookie,
  queueClearAccessTokenCookie,
} from '@core/auth/authCookies';
...
import { PlatformPolicyService } from '@platform/superadmin/platformPolicy/PlatformPolicy.service';
import { AuthUserType, DEFAULT_AUTH_USER_TYPE, isAuthUserType } from '@core/auth/constants/UserTypes.constant';
import { parseExpiresIn } from '@core/auth/parseExpiresIn';
...
export class AuthService {
  ...
  private isRateLimitedLoginFailure(error: unknown): error is DomainError {
    return (
  ...
  private async signAccountToken(
    ...
    return {
      accessToken,
      expiresIn,
      accessTokenExpiresAt: this.calculateAccessTokenExpiresAt(expiresIn),
    };
  }

  private calculateAccessTokenExpiresAt(expiresIn: string): Date {
    const seconds = parseExpiresIn(expiresIn);
    return new Date(Date.now() + seconds * 1000);
  }

  private requestMetadata(req: FastifyRequest | AuthSessionContext['req']): {
    ...
    return {
      ipAddress:
        firstForwarded ||
        req.ip ||
        (typeof req.raw?.socket?.remoteAddress === 'string' ? req.raw.socket.remoteAddress : null),
  ...
  private async listAccountsByType(
    ...
    skip: number,
    take: number,
  ): Promise<{ data: AuthAccountRecord[]; totalCount: number }> {
    const [rows, totalCount] = await this.listRawAccountsByType(userType, skip, take);
    const data = await Promise.all(rows.map((row) => this.enrichAccount(userType, row)));
  ...
  private async listRawAccountsByType(
    ...
    skip: number,
    take: number,
  ): Promise<[AuthAccountRow[], number]> {
    const where = { deletedAt: null };
    const orderBy = { createdAt: 'desc' as const };
    switch (userType) {
      case 'SUPER_ADMIN': {
        const [rows, count] = await Promise.all([
          this.prisma.superAdminUser.findMany({ skip, take, where, orderBy }),
          this.prisma.superAdminUser.count({ where }),
        ]);
        return [rows, count];
      }
      case 'DISTRIBUTOR_USER': {
        const [rows, count] = await Promise.all([
          this.prisma.distributorUser.findMany({ skip, take, where, orderBy }),
          this.prisma.distributorUser.count({ where }),
        ]);
        return [rows, count];
      }
      case 'BRAND_ADMIN': {
        const [rows, count] = await Promise.all([
          this.prisma.brandAdminUser.findMany({ skip, take, where, orderBy }),
          this.prisma.brandAdminUser.count({ where }),
        ]);
        return [rows, count];
      }
      case 'CORPORATE_ADMIN': {
        const [rows, count] = await Promise.all([
          this.prisma.corporateAdminUser.findMany({ skip, take, where, orderBy }),
          this.prisma.corporateAdminUser.count({ where }),
        ]);
        return [rows, count];
      }
      default:
  ...
  private async listAllAccounts(skip: number, take: number): Promise<{ data: AuthAccountRecord[]; totalCount: number }> {
    const unionCte = Prisma.sql`
      SELECT
        sa.id, sa."loginId", sa."passwordHash", sa."displayName", sa.email, sa.phone, sa.status,
        sa."lastLoginAt", sa."passwordChangedAt", sa."createdAt", sa."updatedAt", sa."deletedAt",
        NULL::uuid AS "distributorId", NULL::uuid AS "brandHQId", NULL::uuid AS "corporateId",
        'SUPER_ADMIN'::text AS "userType"
      FROM "SuperAdminUser" sa WHERE sa."deletedAt" IS NULL
      UNION ALL
      SELECT
        du.id, du."loginId", du."passwordHash", du."displayName", du.email, du.phone, du.status,
        du."lastLoginAt", du."passwordChangedAt", du."createdAt", du."updatedAt", du."deletedAt",
        du."distributorId", NULL::uuid AS "brandHQId", NULL::uuid AS "corporateId",
        'DISTRIBUTOR_USER'::text AS "userType"
      FROM "DistributorUser" du WHERE du."deletedAt" IS NULL
      UNION ALL
      SELECT
        bu.id, bu."loginId", bu."passwordHash", bu."displayName", bu.email, bu.phone, bu.status,
        bu."lastLoginAt", bu."passwordChangedAt", bu."createdAt", bu."updatedAt", bu."deletedAt",
        bp."distributorId", bu."brandHQId", NULL::uuid AS "corporateId",
      ...
      UNION ALL
      SELECT
        cu.id, cu."loginId", cu."passwordHash", cu."displayName", cu.email, cu.phone, cu.status,
        cu."lastLoginAt", cu."passwordChangedAt", cu."createdAt", cu."updatedAt", cu."deletedAt",
        NULL::uuid AS "distributorId", NULL::uuid AS "brandHQId", cu."corporateId",
        'CORPORATE_ADMIN'::text AS "userType"
      FROM "CorporateAdminUser" cu WHERE cu."deletedAt" IS NULL
    `;

    const [rows, countResult] = await Promise.all([
      this.prisma.$queryRaw<AuthAccountListRow[]>(Prisma.sql`
        SELECT * FROM (${unionCte}) merged_accounts
        ORDER BY "createdAt" DESC, id DESC
        OFFSET ${skip} LIMIT ${take}
      `),
      this.prisma.$queryRaw<[{ cnt: bigint }]>(Prisma.sql`
        SELECT COUNT(*)::bigint AS cnt FROM (${unionCte}) merged_accounts
      `),
    ]);

    const totalCount = Number(countResult[0]?.cnt ?? 0);
    const data = await Promise.all(rows.map((row) => this.enrichAccount(row.userType, row)));
    return { data, totalCount };
  }
  ...
  async validateUser(input: LoginInput): Promise<AuthAccountRecord> {
    ...
    // SuperAdmin 바이패스: tenant scope 가 빠진 포털 로그인 요청이라도
    // SuperAdminUser 테이블��� 해당 loginId 가 존재하��� SUPER_ADMIN 으로 인증한다.
    const needsScope = userType !== 'SUPER_ADMIN';
    let effectiveUserType = userType;
    if (needsScope) {
      try {
        this.requireScopeForUserType(userType, scope);
      } catch (scopeErr) {
        const superAdminFallback = await this.prisma.superAdminUser.findFirst({
          where: { loginId: input.loginId, deletedAt: null },
        });
        if (superAdminFallback) {
          effectiveUserType = 'SUPER_ADMIN' as AuthUserType;
        } else {
          throw scopeErr;
        }
      }
    ...
    const user = await this.findAccountByLogin(effectiveUserType, input.loginId, scope);
    if (!user || user.deletedAt) {
      throw new DomainError({ code: 'INVALID_CREDENTIALS' });
    }
    ...
    const model = await this.enrichAccount(effectiveUserType, user);

    // SuperAdmin 바이패스로 CorporatePortal 에 로그인한 경우,
    // 같은 loginId 로 등록된 CorporateAdminUser 의 corporateId 를 주입한다.
    if (effectiveUserType === 'SUPER_ADMIN' && userType === 'CORPORATE_ADMIN' && !model.corporateId) {
      const linkedCorporateAdmin = await this.prisma.corporateAdminUser.findFirst({
        where: { loginId: input.loginId, deletedAt: null },
        select: { corporateId: true },
      });
      if (linkedCorporateAdmin) {
        model.corporateId = linkedCorporateAdmin.corporateId;
      }
    }
    ...
    switch (effectiveUserType) {
      ...
      case 'CORPORATE_ADMIN':
        ...
        break;
      default:
        throw new DomainError({ code: 'INVALID_USER_TYPE', params: { userType: effectiveUserType } });
    }
   ...
   */
  async generateToken(user: AuthAccountRecord): Promise<{
    accessToken: string;
    expiresIn: string;
    accessTokenExpiresAt: Date;
  }> {
  ...
  async login(inputOrLoginId: LoginInput | string, password?: string) {
    ...
    try {
      ...
      if (this.isRateLimitedLoginFailure(error)) {
        ...
        if (!rateLimit.allowed) {
          throw new DomainError({
            ...
            details: { limit, windowSeconds, count: rateLimit.count },
          });
        }
      }
      throw error;
  ...
  async loginWithSession(
    ...
    user: AuthAccountRecord;
  }> {
    const user = await this.login(input);
    const session = await this.authSession.issueRefreshSession(
      user.user.userType,
      user.user.id,
      this.requestMetadata(context.req),
    );
    queueRefreshTokenCookie(context.responseCookies, session.refreshToken);

    const token = await this.signAccountToken(user.user, session.sessionId);
    queueAccessTokenCookie(context.responseCookies, token.accessToken, parseExpiresIn(token.expiresIn));
    return {
      ...token,
  ...
  async refreshSession(
    ...
    const refreshToken = readRefreshTokenCookie(context.req);
    if (!refreshToken) {
    ...
    let rotated;
    try {
      rotated = await this.authSession.rotateRefreshSession(
        refreshToken,
        this.requestMetadata(context.req),
      );
    } catch (error) {
    ...
    const user = await this.findById(rotated.userId, rotated.userType);
    if (!user) {
      queueClearRefreshTokenCookie(context.responseCookies);
      throw new DomainError({ code: 'USER_NOT_FOUND', params: { userId: rotated.userId, userType: rotated.userType } });
    }

    // SuperAdmin 이 CorporatePortal 에서 refresh 하면 corporateId 가 null.
    // 연결된 CorporateAdminUser 의 corporateId 를 주입한다.
    if (rotated.userType === 'SUPER_ADMIN' && !user.corporateId) {
      const linked = await this.findLinkedCorporateId(user.loginId);
      if (linked) user.corporateId = linked;
    }

    const token = await this.signAccountToken(user, rotated.sessionId);
    queueAccessTokenCookie(context.responseCookies, token.accessToken, parseExpiresIn(token.expiresIn));
    return {
      ...token,
  ...
  async logoutSession(context: AuthSessionContext): Promise<boolean> {
    const refreshToken = readRefreshTokenCookie(context.req);
    await this.authSession.revokeRefreshSession(refreshToken, 'USER_LOGOUT');
    queueClearRefreshTokenCookie(context.responseCookies);
    queueClearAccessTokenCookie(context.responseCookies);
    return true;
  }

  async logoutAllSessions(user: JwtPayload, context: AuthSessionContext): Promise<boolean> {
    await this.authSession.revokeAllUserSessions(
      user.userType,
      user.sub,
      'USER_LOGOUT_ALL',
      user.sessionId,
    ...
    const refreshToken = readRefreshTokenCookie(context.req);
    await this.authSession.revokeRefreshSession(refreshToken, 'USER_LOGOUT_ALL');
    queueClearRefreshTokenCookie(context.responseCookies);
    queueClearAccessTokenCookie(context.responseCookies);
    return true;
  }
  ...
  /** SuperAdmin loginId 와 동일한 CorporateAdminUser 의 corporateId 를 반환한다. */
  async findLinkedCorporateId(loginId: string): Promise<string | null> {
    const linked = await this.prisma.corporateAdminUser.findFirst({
      where: { loginId, deletedAt: null },
      select: { corporateId: true },
    });
    return linked?.corporateId ?? null;
  }
   ...
   */
  async findAll(skip: number, take: number, userType?: string): Promise<{ data: AuthAccountRecord[]; totalCount: number }> {
    if (userType) {
      const type = this.normalizeUserType(userType);
