...
import { LicenseService } from '@platform/superadmin/license/License.service';
...
export class BranchService {
  ...
  private evalCtxFor(user: JwtPayload, brandHqId?: string | null, branchId?: string | null) {
    return {
  ...
  async findAll(skip: number, take: number, user?: JwtPayload) {
    const where = combineWhere<Prisma.BranchWhereInput>(
      { deletedAt: null },
      branchScopeWhere(user),
    );
    const [data, totalCount] = await Promise.all([
      this.prisma.branch.findMany({ where, skip, take, orderBy: { createdAt: 'desc' } }),
  ...
  async create(input: CreateBranchInput, user?: JwtPayload) {
    ...
    if (user) await this.permission.require(this.evalCtxFor(user, brand.id), 'brand.branch.write');
    await this.licenseService.enforceBranchLimit(brand.id);
  ...
  async update(id: string, input: UpdateBranchInput, user?: JwtPayload) {
    const branch = await this.ensureAccessibleBranch(id, user);
    if (user) await this.permission.require(this.evalCtxFor(user, branch.brandHQId, branch.id), 'brand.branch.write');
