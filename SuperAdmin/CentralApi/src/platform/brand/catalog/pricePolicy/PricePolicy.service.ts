...
export class PricePolicyService {
  ...
  private async ensureAccessiblePolicy(id: string, user?: JwtPayload) {
    ...
    if (!policy) {
      throw new DomainError({ code: 'CROSS_TENANT_ACCESS_DENIED', details: { reason: 'Price policy is outside caller scope' } });
    }
  ...
  async softDelete(id: string, user?: JwtPayload) {
    const policy = await this.ensureAccessiblePolicy(id, user);
    if (user) await this.permission.require(this.evalCtxFor(user, policy.brandHQId), 'brand.catalog.write');
