...
export function EmployeeListScreen() {
  ...
  const totalCount = data?.mealEmployees?.success?.totalCount ?? employees.length;
  ...
  return (
    <DetailPageTemplate
      ...
      summaryItems={[{ label: t('nav.employees'), value: totalCount, tone: 'brand' }]}
    >
      {showForm && canWrite && (
        <div className="mb-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <Select
                    ...
                    onChange={(val) => setEmploymentType(val as string)}
                    options={[
                      { value: 'FULL_TIME', label: '정규직' },
                      { value: 'CONTRACT', label: '계약직' },
                      { value: 'DISPATCH', label: '파견직' },
                      { value: 'CONTRACTOR_AGENCY', label: '도급' },
                    ]}
                  />
                </div>
              </div>
      ...
      <SectionCard title={t('nav.employees')} description={`${totalCount}명`} padding="none">
        {loading ? (
