...
import { formatVnd, formatTime } from '../../mockData';
...
export function TransactionCard({ transaction }: TransactionCardProps) {
  ...
  return (
    <div className="flex items-center gap-3 bg-white rounded-xl p-3">
      {/* Avatar */}
      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-white text-sm font-semibold ${avatarColor}`}>
        {initial}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-400 mt-0.5">{formatTime(transaction.createdAt)}</p>
      </div>

      {/* Right */}
      <div className="flex flex-col items-end shrink-0">
        <p className={`text-sm ${amount.className}`}>{amount.text}</p>
        {isDeclined ? (
          <span className="mt-0.5 inline-flex items-center rounded-full bg-[#EF4444]/10 px-2 py-0.5 text-[10px] font-medium text-[#EF4444]">
