...
export function SettingsSessionScreen() {
  ...
  // TODO: populate from auth state / user query
  const loading = true;

  // Section 1: Profile
  const [fullName, setFullName] = useState('');
  const [email] = useState(''); // readonly
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    // TODO: mutation changePassword({ currentPassword, newPassword })
    setCurrentPassword('');
    setNewPassword('');
    setConfirmPassword('');
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5 md:col-span-2">
            ...
            {loading ? (
              ...
              >
                {(Object.keys(TIMEZONE_LABELS) as Timezone[]).map((key) => (
                  <option key={key} value={key}>
                    {TIMEZONE_LABELS[key]}
                  </option>
                ))}
