...
| 항목 | 값 |
|------|---|
| 문서 ID | SCR-PAY-ZALOPAY-QR |
| 화면 경로 | `screens/PaymentScreen` (QRPaymentModal 오픈) |
| 레거시 다이얼로그 | `IDD_ZALOPAY_QR` (리소스 441) |
...
ZaloPay QR 결제 모달이다. 결제 금액에 대한 QR 코드를 생성하여 고객이 ZaloPay 앱으로 스캔/결제하고, POS가 승인 결과를 확인하는 동기 대기(synchronous-wait) 방식 결제 화면이다.

- **결제 방식**: 동기 대기 -- 외부 승인 결과를 받은 후에만 DB에 저장한다
- **Outbox 대상 여부**: 아니다. QR 결제 승인 요청은 Outbox 재전송 대상이 아니다
- **온라인 필수**: 오프라인 시 UseCases 진입을 차단한다
- **공통 컴포넌트**: `shared/ui/organisms/QRPaymentModal` (gateway adapter: `zalopay`)
...
| 데이터 흐름 | QR 생성 요청 -> C++ ExternalBridge/ZaloPay -> 승인 대기 -> DB 저장 -> PosRealTimeSender -> UI 갱신 |
...
|---|---|---|---|---|---|---|
| ZP-F01 | QR 코드 생성/표시 | `PAYMENT:QR:GENERATE` | `ExecuteQRPaymentUseCase` | `SaleMgr` | `ExternalBridge/PaymentGateways/ZaloPay` | P0 |
| ZP-F02 | 결제 상태 수동 확인 | `PAYMENT:QR:CHECK_STATUS` | `CheckQRPaymentStatusUseCase` | `SaleMgr` | `ExternalBridge/PaymentGateways/ZaloPay` | P0 |
| ZP-F03 | QR 코드 취소 | `PAYMENT:QR:CANCEL` | `CancelQRPaymentUseCase` | `SaleMgr` | `ExternalBridge/PaymentGateways/ZaloPay` | P0 |
| ZP-F04 | 결제 완료 확정 (자동) | `PAYMENT:QR:CONFIRM` | `ExecuteQRPaymentUseCase` | `SaleMgr` | `SQLite/Tables/Payment/WaitPaymentCrud`, `SellSlipCrud` | P0 |
| ZP-F05 | 결제 상태 자동 폴링 | `PAYMENT:QR:CHECK_STATUS` | `CheckQRPaymentStatusUseCase` | `SaleMgr` | `ExternalBridge/PaymentGateways/ZaloPay` | P0 |
...
screens/PaymentScreen
  └─ shared/ui/organisms/QRPaymentModal (gateway="zalopay")
...
|---|---|---|---|
| ZP-D01 | QR 결제 안내 타이틀 | QRPaymentModal 헤더 | i18n msgKey (`payment.qr.title`, params: `{gateway: "zalopay"}`) |
| ZP-D02 | QR 코드 이미지 | QRPaymentModal QR 영역 | `paymentApi.generateQR` mutation 응답 |
| ZP-D03 | 결제 상태 텍스트 | QRPaymentModal 상태 영역 | `paymentApi.checkQRStatus` polling 응답 |
| ZP-D04 | 결제 금액 | QRPaymentModal 금액 영역 | `paymentApi.generateQR` 응답 내 금액 |
...
|---|---|---|---|
| `PAYMENT:QR:GENERATE` | UI -> C++ | `{ gateway: "zalopay", amount, tableCode, idempotencyKey }` | `{ qrImageUrl, transactionId, expiresAt }` |
| `PAYMENT:QR:CHECK_STATUS` | UI -> C++ | `{ gateway: "zalopay", transactionId }` | `{ status: "PENDING"\|"SUCCEEDED"\|"FAILED", amount }` |
| `PAYMENT:QR:CANCEL` | UI -> C++ | `{ gateway: "zalopay", transactionId }` | `{ cancelled: boolean }` |
| `PAYMENT:QR:REFRESH` | UI -> C++ | `{ gateway: "zalopay", transactionId }` | `{ qrCode, newExpiresAt }` |
...
| 엔드포인트 | 타입 | 태그 | 비고 |
|---|---|---|---|
| `paymentApi.generateQR` | mutation | -- | gateway param으로 ZaloPay 분기 |
| `paymentApi.checkQRStatus` | query (polling) | -- | `pollingInterval` 설정으로 자동 폴링 |
| `paymentApi.cancelQR` | mutation | -- | |

### 5.3 UseCase 흐름

1. `ExecuteQRPaymentUseCase`: 오프라인 체크 -> Ledger `RECEIVED` 기록 -> ExternalBridge/ZaloPay QR 생성 API 호출 -> Ledger `PROCESSING` -> 응답 반환
2. `CheckQRPaymentStatusUseCase`: ZaloPay 상태 조회 -> 성공 시 DB 저장 (WaitPayment, SellSlip, SellDetail) -> Ledger `SUCCEEDED` -> PosRealTimeSender로 UI 갱신 이벤트 방송
3. `CancelQRPaymentUseCase`: ZaloPay 취소 API 호출 -> Ledger `COMPENSATED` 기록

### 5.4 PosRealTime 이벤트

| 이벤트 | 발행 시점 | Payload |
|---|---|---|
| `PAYMENT:QR:STATUS_CHANGED` | 결제 상태 변경 시 (UseCase에서 발행) | `{ transactionId, status, amount, gateway: "zalopay" }` |
| `PAYMENT:QR:COMPLETED` | 결제 확정 후 DB 커밋 완료 시 | `{ transactionId, sellSlipId, amount, gateway: "zalopay" }` |
...
| ID | 시나리오 | 검증 항목 |
|---|---|---|
| ZP-T01 | QR 생성 성공 | QR 이미지 렌더링, 금액 표시, 폴링 시작 |
| ZP-T02 | 결제 승인 성공 (폴링 자동 확정) | DB 저장 확인 (WaitPayment, SellSlip), Ledger SUCCEEDED, UI 완료 표시 |
| ZP-T03 | QR 취소 | ZaloPay 취소 API 호출, Ledger COMPENSATED, 모달 닫기 |
| ZP-T04 | 오프라인 진입 차단 | 네트워크 미연결 시 QR 생성 mutation 차단, 안내 메시지 |
| ZP-T05 | QR 만료 | expiresAt 경과 후 자동 취소 처리 |
| ZP-T06 | 결제 실패 | Ledger FAILED, 사용자 안내, 재시도 가능 |
| ZP-T07 | 언어 전환 | KR/EN/VN 전환 시 모달 내 모든 텍스트 i18n 반영 |
| ZP-T08 | mockTransport 동작 | `next dev` + mockTransport 조합으로 C++ 없이 QR 모달 동작 확인 |

## 7. 완료기준

- [ ] `shared/ui/organisms/QRPaymentModal` 공통 컴포넌트에서 `gateway="zalopay"` 분기 동작
...
- [ ] `ExternalBridge/PaymentGateways/ZaloPay` 어댑터 구현
...
| 2 | paymentApi RTK Query 엔드포인트 (generateQR, checkQRStatus, cancelQR) | BrandPosApp/PosUi/store/api | TODO |
