...
/**
 * InfoplusWooriQRDialog (SCR-PAY-INFOPLUS-WOORI-QR)
 *
 * Infoplus gateway -> Woori bank QR payment modal.
 * Synchronous-wait with auto-polling. Online required. NOT Outbox target.
 * Shares Infoplus protocol with Shinhan/BIDV via bankCode param.
 *
 * Legacy: IDD_INFOPLUS_WOORI_QR (453), 414x327 DLU, 11 controls
 * Uses shared/ui/organisms/QRPaymentModal with gateway="infoplus", bankCode="WOORI"
 */
...
interface InfoplusWooriQRDialogProps {
  open: boolean;
  onClose: () => void;
  amount: number;
  tableCode: string;
...
const BANK_CODE = 'WOORI';
const POLLING_INTERVAL_MS = 3000;
const POLLING_TIMEOUT_MS = 5 * 60 * 1000;

// --- Component ---

export default function InfoplusWooriQRDialog({
  open,
  onClose,
  amount,
  tableCode,
}: InfoplusWooriQRDialogProps) {
  ...
  const handleGenerateQR = useCallback(() => {
    ...
    //       Bridge PAYMENT:QR:GENERATE { gateway: "infoplus", bankCode: "WOORI", amount, tableCode, idempotencyKey }
    //       -> ExecuteQRPaymentUseCase -> ExternalBridge/Infoplus (bankCode=WOORI)
  }, []);

  const handleCheckStatus = useCallback(() => {
    // TODO: paymentApi.checkQRStatus polling
    //       Bridge PAYMENT:QR:CHECK_STATUS { gateway: "infoplus", bankCode: "WOORI", transactionId }
  }, []);

  const handleCancel = () => {
    // TODO: paymentApi.cancelQR mutation
    //       Bridge PAYMENT:QR:CANCEL { gateway: "infoplus", bankCode: "WOORI", transactionId }
    setStatus('CANCELLED');
    onClose();
  };
  ...
  return (
    <Modal open={open} onClose={handleCancel} title="Woori QR (Infoplus)" size="md">
      <div className="flex flex-col items-center p-6 gap-4">
        <div className="w-48 h-48 border-2 border-pos-border rounded-lg flex items-center justify-center bg-white">
          ...
          {qrImageUrl ? (
            <img src={qrImageUrl} alt="Woori QR" className="w-full h-full object-contain" />
          ) : status !== 'GENERATING' ? (
        ...
        <div className="text-sm text-center">
          ...
          {status === 'PENDING' && 'Waiting for Woori payment...'}
