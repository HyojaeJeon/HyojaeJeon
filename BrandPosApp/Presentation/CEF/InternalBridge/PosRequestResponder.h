...
public:
    /**
     * cefQuery 콜백에서 호출한다.
     * @param requestJson  UI가 보낸 JSON 문자열
     * @param onSuccess    성공 콜백 (응답 JSON 문자열)
     * @param onFailure    실패 콜백 (에러코드, 에러메시지)
     */
    static void HandleRequest(
        const std::string& requestJson,
        std::function<void(const std::string& responseJson)> onSuccess,
        std::function<void(int errorCode, const std::string& errorMsg)> onFailure);
...
private:
     ...
     * @param idempotencyKey  멱등성 키 (빈 문자열 가능)
