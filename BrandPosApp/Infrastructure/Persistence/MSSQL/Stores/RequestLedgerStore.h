...
struct LedgerEntry {
    ...
    std::string resultJson;   // 성공 시 응답 데이터
...
public:
     ...
     */
    std::optional<LedgerEntry> Find(const std::string& idempotencyKey);

    /**
     * 신규 엔트리 삽입 또는 기존 엔트리 갱신
     */
    void Upsert(const std::string& idempotencyKey,
     ...
     * 상태 갱신
     */
    void UpdateStatus(const std::string& idempotencyKey,
     ...
     */
    std::vector<LedgerEntry> FindStaleProcessing(int timeoutSeconds = 300);
};
