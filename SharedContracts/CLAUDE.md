...
- `ApiSdk`의 GraphQL operation export는 schema drift 검증과 persisted manifest 생성이 직접 읽을 수 있게 **평면 shape** 를 유지한다. 최소 `operationName` 과 `document` 를 top-level 필드로 노출한다.
- operation validator 가 `0 operations` 로 통과하는 상태를 허용하지 않는다. build gate 는 실제 operation export 를 검출하지 못하면 실패해야 한다.
