...
export function useUpload(options) {
    const { category, baseUrl: explicitBaseUrl, getToken, maxSize, allowedTypes, onSuccess, onError, } = options;
    ...
    const upload = useCallback(async (file) => {
        ...
        try {
            ...
            const result = await uploadFile({
                ...
                maxSize,
                allowedTypes,
                onProgress: (percent) => setProgress({ total: 1, completed: 0, currentFilePercent: percent }),
    ...
    const rollback = useCallback(async (keys) => {
        const token = await resolveToken();
