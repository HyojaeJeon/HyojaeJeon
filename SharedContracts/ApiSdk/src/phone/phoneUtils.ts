 ...
 */
export function stripLeadingZero(localNumber: string): string {
 ...
 */
export function validatePhone(e164: string): 'INVALID_FORMAT' | 'UNSUPPORTED_COUNTRY' | 'TOO_SHORT' | 'TOO_LONG' | null {
  if (!e164.startsWith('+')) return 'INVALID_FORMAT';

  const parsed = parseE164(e164);
  if (!parsed) return 'UNSUPPORTED_COUNTRY';

  const country = PHONE_COUNTRIES.find((c) => c.code === parsed.countryCode)!;
  if (parsed.localNumber.length < country.minDigits) return 'TOO_SHORT';
  if (parsed.localNumber.length > country.maxDigits) return 'TOO_LONG';
  if (!country.regex.test(e164)) return 'INVALID_FORMAT';
 ...
 */
export function normalizePhone(raw: string, defaultCountry: string = 'VN'): string {
